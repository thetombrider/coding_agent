...
- **Role-bound model routing for subagents.** Each `task` subagent preset is bound to a model rather than always running on the main model. The lead model's existing choice of preset *is* the content classification — no separate classifier. Defaults: `explore` → **cheap** tier (read-only investigation: context-heavy, light reasoning — the configured cheap model, deepseek-v4-flash by default), `implement` → a **code-tuned** model (`moonshotai/kimi-k2.7-code` by default), `review` → **main** tier. Resolution happens in `resolvePresetModel` (`src/config/models.ts`) against the *live* session models, so a runtime `/model` switch is honoured for the tier-based roles. Precedence: an explicit per-role override in config (`models.roles.<preset>`) > the role's built-in model default (e.g. the code model for `implement`) > the tier default. The first two apply **only when the active provider supports that id**; otherwise the role falls back to its tier default, so non-OpenRouter providers degrade cleanly. Example config:
  ```jsonc
  // ~/.orin/config.json
  "models": { "roles": { "implement": "qwen/qwen3-coder-next" } }
  ```
