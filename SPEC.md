...
**Further reading (patterns study, not primary references):** **Cline** and **Kilo Code** (Roo lineage) — see [`docs/reference/cline-kilo.md`](docs/reference/cline-kilo.md) for patterns mapped to these phases (robust diff editing → Phase 6, orchestrator/subtasks → Phase 8, shadow-git checkpoints, context condensing → Phase 7). **Charm Crush** — see [`docs/reference/crush.md`](docs/reference/crush.md) for LSP integration, hooks/permissions UX, client/server headless embedding, and TUI patterns (issue #327). **Command Code** — see [`docs/reference/command-code.md`](docs/reference/command-code.md) for its Taste learning model and a review-gated preference-ledger design for Orin (issue #413).
...
- **Role-bound model routing for subagents.** Each `task` subagent preset is bound to a model rather than always running on the main model. The lead model's existing choice of preset *is* the content classification — no separate classifier. Defaults: `explore` → **cheap** tier (read-only investigation: context-heavy, light reasoning — the configured cheap model, deepseek-v4-flash by default), `implement` → a **code-tuned** model (`moonshotai/kimi-k2.7-code` by default), `review` → **main** tier. Resolution happens in `resolvePresetModel` (`src/config/models.ts`) against the *live* session models, so a runtime `/model` switch is honoured for the tier-based roles. Precedence: an explicit per-role override in config (`models.roles.<provider>.<preset>`) > the role's built-in model default (e.g. the code model for `implement`) > the tier default. The first two apply **only when the active provider supports that id**; otherwise the role falls back to its tier default, so non-OpenRouter providers degrade cleanly. Per-role overrides are **provider-scoped** (`providerId → role → model`) so a role stays pinned to a model the provider actually supports; switching providers restores that provider's own choices. The `/settings` menu writes these under the *active* provider, and a legacy flat `role → model` config is migrated onto the active provider on read. Example config:
  ```jsonc
  // ~/.orin/config.json
  "models": { "roles": { "openrouter": { "implement": "qwen/qwen3-coder-next" } } }
  ```
- **Subagent isolation (where a `task` subagent's edits land).** Three modes, resolved in `resolveIsolation` (`src/tools/task.ts`); the preset sets the default. **`shared`** (default for every preset, including the mutating `implement`): the child runs against the host `Workspace`/`cwd` and **edits persist to the local working tree** — matching Claude Code / opencode / pi / nanocoder. Safe because subagents are serial (depth-1, parent pauses). **`worktree`** (opt-in): the child runs in a `git worktree` on a fresh `orin/subagent-<id>` branch (`src/workspace/worktree.ts`); edits are isolated from the host tree but **persist to that branch**, and the summary reports the branch + `git diff --stat`. **`sandbox`** (opt-in, requires `sandbox.e2b.apiKey` in config): the existing ephemeral E2B clone — isolated, but its edits are **discarded on dispose** (no flow-back), so it is for untrusted code, not routine implementation. The E2B/`Workspace`/seed machinery is retained as the foundation for whole-session E2B (`sandbox.active`) and detached/cloud agents (Phase 9). Pairs with shadow-git checkpoints (cheap local undo) for the `shared` default. The user sets a **floor** via `subagent.isolation` in config (`/settings isolation` in the TUI), default `shared`; it is a guarantee, not just a default — `resolveIsolationMode` (`src/agent/isolation.ts`) lets the lead model *escalate* a given task to a more-isolated mode but never weaken below the floor (read-only presets are exempt, so a `sandbox` floor never forces a costly clone for `explore`/`review`).
...
**9b — E2B adapter** (`src/workspace/e2b.ts`). Package `e2b`; needs `sandbox.e2b.apiKey` in config. `commands.run` streams natively, which maps cleanly onto `onData`.
```ts
...
- **Done when:** with `--sandbox e2b` (and `sandbox.e2b.apiKey` set in config), the agent clones a repo into the sandbox, edits a file, runs its tests, and reports results — with zero commands touching the local machine.
