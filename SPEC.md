...
- **Role-bound model routing for subagents.** Each `task` subagent preset is bound to a model **tier** rather than always running on the main model. The lead model's existing choice of preset *is* the content classification — no separate classifier. Defaults: `explore` → **cheap** (read-only investigation: context-heavy, light reasoning), `review` → **main**, `general` → **main**. Resolution happens in `resolvePresetModel` (`src/config/models.ts`) against the *live* session models, so a runtime `/model` switch is honoured. Precedence: an explicit per-role override in config (`models.roles.<preset>`) wins **when the active provider supports that id**; otherwise it falls back to the tier default. Tier defaults map to the host's `main`/`cheap` models. Example config:
  ```jsonc
  // ~/.orin/config.json
  "models": { "roles": { "explore": "deepseek/deepseek-v4-flash" } }
  ```
