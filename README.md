...
(subagents), `todowrite` (a live plan), and `askuser` (pause to ask the user a
multiple-choice question). The provider layer wraps the AI SDK's
...
src/
  ...
  tools/          # read, write, edit, bash, grep, find, ls, delegate_read, task, todowrite, askuser (+ .txt descriptions)
