...
- **`OPENROUTER_API_KEY` is needed for a few tests.** `bun run test` is otherwise self-contained, but `src/delegate/delegate-read.test.ts` and `src/agent/compaction.test.ts` build the model argument eagerly via the provider registry (`resolveLanguageModel` → the active provider's `languageModel()`, which for OpenRouter calls `getOpenRouter()`) even though they inject a mock generator, so they throw without the env var. Any non-empty value works for the suite (e.g. `OPENROUTER_API_KEY=dummy bun run test`); a real key is only required to actually hit the model. The provider-registry tests (`src/provider/registry.test.ts`) manage their own env and need no key.
