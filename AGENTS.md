...
- **Symbol index cache lives in `~/.orin/index/<repo-hash>.json`.** The Tree-sitter symbol index is persisted per workspace root (hash of the resolved cwd). On startup Orin loads cached symbols when file mtimes and content hashes match; changed files are re-indexed incrementally. Corrupt or missing cache files fall back to a full warm scan. Edits during a session update the cache entry for that file.
