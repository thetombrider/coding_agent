...
`minicoder` is a single-package terminal coding-agent CLI (Bun + SolidJS via `@opentui`). Standard scripts live in `package.json` (`dev`, `build`, `typecheck`, `test`).
...
- **Offline demo without a key:** run `bun src/cli.ts --faux` for a fully offline interactive TUI (scripted responses). Real interactive use needs a real `OPENROUTER_API_KEY` (env var or `~/.coding-agent/config.json`). The TUI needs a real TTY, so run it in a terminal emulator (not piped).
- **Sessions persist to `~/.coding-agent/sessions/<id>.jsonl`.** Each session is its own append-only JSONL log; switching/archiving a session just rotates to a new file, and old logs remain browsable via `/sessions` (or `minicoder --list-sessions`).
